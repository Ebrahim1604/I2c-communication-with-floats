...
/*
/* Note:
 * sen_values_from_master is the array which stores the number of variables
 * sen_values_from_master[0] is always the number of sensor values sent by 
 * the master. sensor values are stored from index 1 to 7, depending
 * on the number of values sent.
 */

double* sen_values_from_master; //array which saves values received from master

double* update_datatomaster(void);
static double datatomaster[8]; //array which saves sensor values connected to slave controller

int incoming_byte = 0;

int request_from_master = 0;

bool sensor_val_updated_from_master = 0;
...
void loop() {
  ...
  if(sensor_val_updated_from_master == 1)
    {
      //noInterrupts();
      Serial.println("Master----->SLAVE");
      sen_values_from_master = get_latest_values();
      Serial.print("Following ");
      Serial.print((int)sen_values_from_master[0]);
      Serial.println(" Sensor values have been received: ");
      
      for(int i=1; i<(int)(sen_values_from_master[0]+1); i++)
      {
        Serial.print("Val");
        Serial.print(i);
        Serial.print(" = ");
        Serial.println(sen_values_from_master[i]);
        }
      sensor_val_updated_from_master = 0;
      }
  //sen_values_from_master[1],sen_values_from_master[2]... can be used in your code further
    ...
    if (Wire.read() == 0x02)
        {
          ...
          if(second_byte == 0x03)
           ...
           else if(second_byte == 0x04) //master to slave
            {
              Serial.println("Receiving data from master....");
              handle_data_sending();
              sensor_val_updated_from_master = 1;
              }
           else if(second_byte == 0x05)//slave to master
           {
              Serial.println("SLAVE----->Master");
              Serial.println("Request for Data received....");
              data_req_rcvd = 1;
              sen_values_from_master = update_datatomaster();
              Serial.print("Following ");
              Serial.print((int)sen_values_from_master[0]);
...
double* update_datatomaster(void) //Values acquired by the sensors connected to slave
{
  int no_of_values = 4;// define the number of sensor values you are sending (max 7)
  
    //replace these values actual values
    //Note: The size of this array is 8,so maximum 7 values can sent to the master
...
void requestEvent()
{
  if (data_req_rcvd == 1) //data req from slave to master
  {
  ...
  {
    request_from_master = 1; //authentication requested
    return;
  }
}
